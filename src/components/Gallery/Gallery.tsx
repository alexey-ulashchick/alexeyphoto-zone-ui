...
import { ImageStyle, GalleryScrollableContainerStyle, DebuggingDiv, GalleryContainer } from './GalleryStyle';
    ...
    return (
      <div id={`${hash}_container`} className={GalleryContainer}>
        <div id={`${hash}_scrollableContainer`} className={GalleryScrollableContainerStyle}>
          ...
          {list.map((deg, index) => (
            <div id={`${hash}_${index}`} className={ImageStyle} key={index} style={{ transform: `rotate(${0}deg)` }}>
              {index}
            </div>
          ))}
        </div>
